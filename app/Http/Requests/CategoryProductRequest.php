...
class CategoryProductRequest extends FormRequest
{
    ...
    public function rules()
    {
        switch ($this->method()) {
            ...
            case 'POST': {
                return [
                    'name'        => 'required|unique:categories_product,name,NULL,id,deleted_at,NULL',
                ];
                break;
            }
            case 'PUT':
            case 'PATCH': {
                return [
                    'name'  => 'required|unique:categories_product,name,' . $this->route('category_product') . ',id,deleted_at,NULL',
                ];
                break;
            }
            default:
